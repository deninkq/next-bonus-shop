  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loginSuccess) {
        ...
        setErrors({
          api: translations("errors.notFound"),
        });
      }
    } catch (error) {
      console.log(error);
      setErrors({
        api: translations("errors.serverError"),
      });
    } finally {
      setIsSubmitting(false);
    }
      ...
      <StyledPaper elevation={10}>
        ...
        <form onSubmit={handleSubmit}>
          <TextField
            ...
            onBlur={handleBlur}
            error={(!!errors.username && touched) || !!errors.api}
            sx={{ marginBottom: errors.username || errors.api ? 0 : 2 }}
            disabled={isSubmitting}
          />
          {errors.username && touched && (
            <ErrorText>{errors.username}</ErrorText>
          )}
          {errors.api && hasSubmitted && <ErrorText>{errors.api}</ErrorText>}
          <Button
            type="submit"
            variant="contained"
            fullWidth
